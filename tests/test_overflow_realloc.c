...
int main(void) {
    ...
    /* Initial allocation - volatile prevents compiler overflow analysis */
    volatile char *buf = malloc(50);
    if (!buf) {
        fprintf(stderr, "malloc failed\n");
        return 1;
    }
    memset((void *)buf, 'A', 50);

    /* Grow with realloc */
    buf = realloc((void *)buf, 200);
    if (!buf) {
